...
ko.bindingHandlers.flatpickr = {
    init: function (element, valueAccessor, allBindingsAccessor) {
        var options = allBindingsAccessor().flatpickrOptions || {}; // Get any custom options
        var observable = valueAccessor(); // Get the bound observable

        // Initialize Flatpickr on the element
        var flatpickrInstance = flatpickr(element, options);

        // Subscribe to changes in the observable
        observable.subscribe(function (newDate) {
            if (newDate instanceof Date) {
                // Update the Flatpickr selected date
                flatpickrInstance.setDate(newDate);
            }
        });

        // Update the observable when the Flatpickr date changes
        flatpickrInstance.config.onChange.push(function (selectedDates, dateStr) {
            observable(new Date(dateStr));
        });
    },
    update: function (element, valueAccessor) {
        var value = ko.utils.unwrapObservable(valueAccessor());

        // Set the selected date on the Flatpickr instance
